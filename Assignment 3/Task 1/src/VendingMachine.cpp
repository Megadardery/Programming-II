...
// copying the given items and coins to the machine
...
// calculating the remainder of the given money by backtracking and trying all possible cases
...
// calls the recursive function but with different parameters
bool VendingMachine::convertMoney(int curr, Money& ans) const
{
	ans = Money(0);
	return convertMoneyRecurse(curr, 0, Money::COIN_COUNT - 1, ans);
}
// checking the item is valid to buy (index , available quantity) , then gives the machine the money and returns the change
